...
Rook::Rook(std::string StartingTile, char team) : ChessPiece(StartingTile, team)
{
	this->PieceTexture.loadFromFile(team == 'W' ? "Images/WhiteRook.png" : "Images/BlackRook.png");
	this->PieceSprite.setTexture(this->PieceTexture);
}

bool Rook::IsLegalMove(const std::vector <std::vector <Tile>>& Tiles, const int x, const int y)
{
